...
intersect nil _ = nil
intersect (h1::t1) l2 = ite (member h1 l2)
                    (h1::intersect t1 l2)
                    (intersect t1 l2)
...
set_forall p (mkSet l1) = (not (member false (map p l1)))
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
set_exists p (mkSet l1) = (member true ( map p l1))
...
list_master: a -> list b -> list (pair a b)
list_master e nil = nil
list_master e (h::t) = (mkPair e h)::(list_master e t)

list_product: list a -> list b -> list (pair a b)
list_product (ha::ta) l2 = (list_master ha l2) ++ (list_product ta l2)
...
subset_elements (h::t) l2 = and (member h l2)
...
eql_set (mkSet s1) (mkSet s2) = same_elements  s1 s2
...
set_toString (mkSet (h::t)) = (toString h) ++ ", "++ (toStringList t)
